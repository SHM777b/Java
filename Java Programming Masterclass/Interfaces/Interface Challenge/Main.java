...
2.  Player (class)
    ...
    -  A constructor that accepts a String (name) and two ints (hitPoints and strength). It initialises name, hitPoints
    ...
    -  And eleven methods:
        ...
        -  toString(), Players overriding toString() method. It takes no arguments and returns a String in the following
        format:
...
3.  Monster (class)
    ...
    -  A constructor that accepts a String (name) and two ints (hitPoints and strength). It initialises name, hitPoints
       and strength with the newly passed in values.
    -  And six methods:
        -  Only getters for the three fields.
        -  write(), same as interface. Return a List of the fields in the order they appear in toString().
        -  read(), same as interface. Store the values in the List, in the order they appear in toString(). Make sure
        the List is not null and the size() is greater than 0 before storing the values.
        -  toString(), Monsters overriding toString() method. It takes no arguments and returns a String in the
        following format:
...
NOTE:  If you get an error from the Evaluate class, it's most likely the constructor. Check if you've added a
    ...
    public static void main(String[] args) {
        ISaveable tim = new Player("Tim", 12, 70);
        List<String> savedGame = new ArrayList<>();
        savedGame = tim.write();
        System.out.println(savedGame);

        List<String> loadableGame = new ArrayList<>();
        loadableGame.addAll(List.of("James Bond", "Pistol", "99", "99"));
        tim.read(loadableGame);
        savedGame = tim.write();
        System.out.println(savedGame);
